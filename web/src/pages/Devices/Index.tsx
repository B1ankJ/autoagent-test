...
export function DevicesPage() {
  ...
  const sessionColumns: ColumnsType<DeviceSession> = [
        {
          title: 'Session ID',
  ...
  const deviceColumns: ColumnsType<Device> = [
    ...
    },
    {
      title: '型号',
      dataIndex: 'model',
      render: (value?: string) => (value ? <span className="aa-mono aa-muted">{value}</span> : '-'),
    },
    {
      title: 'Android',
    ...
    },
    {
      ...
      render: (_, row) => (
        <Space size={4}>
          ...
          <Tag color={row.enabled ? 'blue' : 'red'}>{row.enabled ? 'enabled' : 'disabled'}</Tag>
        </Space>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, row) => (
        <Space size={4} wrap>
          <Button size="small" disabled={!row.online} onClick={() => setStreamSerial(row.serial)}>
            查看画面
          </Button>
          <Button
            size="small"
