...
- Review generation now keeps raw Android candidates instead of collapsing them into a single summarized locator too early. For `input_locator`, `input_focus_action`, and `send_action`, every bounded node is preserved for manual review and recommendation affects ordering only; `latest_bubble_match` review is only emitted when response-block ambiguity remains after structural ranking.
- `send_action` candidate generation now also keeps tap-only visual send controls when the UI tree exposes the real send icon/label as a non-clickable node. This matters for WebView apps where the XML often marks the visible send affordance as `clickable="false"` even though tapping its bounds succeeds.
