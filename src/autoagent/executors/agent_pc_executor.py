        ...
        for prompt in sample.prompts:
            ...
            if copy_cfg and copy_cfg.enabled:
                ...
                for step in clip_result.loop_result.steps:
                    copy_label = f"copy_step_{len(responses) + 1}_{step.step}"
                    step_path = store.write_screenshot(
                        copy_label, base64.b64decode(step.screenshot.base64_data)
                    )
                    ctx.screenshot_index.append(
                        ScreenshotResult(path=step_path, label=copy_label)
                    )
