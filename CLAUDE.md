...
- **Plan 4 — Android Executor (uiautomator2 + OCR):** in progress. Tier 1 backend/device discovery, `/devices` API + page, `gui_android` scheduling, Android profile connectivity, and SampleDetail device/replay metadata are in the repo. Remaining Tier 1 blocker is the fake-chat APK build artifact plus final real-device verification/tagging. Tier 2 target remains OCR, long-response stitching, and `pixel_stable`.
