...
- **Profile Builder review completeness:** do not collapse plausible Android builder candidates away before review. Preserve raw candidate boxes for `input_locator`, `input_focus_action`, `send_action`, and `latest_bubble_match` whenever possible so the UI can show all evidence and let the user decide.
...
- **Strict bounds containers:** when `response_container_locator` is a bounds-based XPath, runtime extraction must not fall back to whole-page scanning if that exact container is missing. Persist the extraction XML (`after_result_<n>.xml`) and log whether the reviewed container matched so real-device failures can be diagnosed from artifacts instead of screenshots alone.
