...
- **Plan 6 — Agent Executor (agent_pc + agent_android):** in progress (started 2026-05-06). Current status: profile schemas, core device abstractions, `PcDevice`, `AndroidDevice`, `ModelClient`, and the current action parser are in the repo; the shared agent loop, screenshot extractor, executor wrappers, and `_deps.py` registration are still incomplete. Source of truth: `docs/superpowers/plans/2026-05-06-agent-executor.md`.
