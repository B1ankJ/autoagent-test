...
- **Response anchors:** treat `response_container_locator` as a structural anchor, not a frozen frame. Runtime extraction should rank matching containers, prefer the latest valid bubble inside the best container, and persist `after_result_<n>.xml` plus container-matching logs for device regressions.
