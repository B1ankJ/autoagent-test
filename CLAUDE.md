...
- **Plan 4 — Android Executor (uiautomator2 + OCR):** in progress. Tier 1 and Tier 2 implementation are in the repo: backend/device discovery, `/devices` API + page with IME enable/disable buttons, `gui_android` scheduling, Android profile connectivity, Android Profile Builder MVP, SampleDetail device/replay metadata, OCR extraction, long-response stitching, `pixel_stable`, opt-in LLM response extraction (`llm_responses` / `llm_errors`, Config connectivity test, Builder inject toggle, SampleDetail dual-view), and Builder `rule` / `smart` draft modes. Current status: backend fast suite plus frontend `test` / `lint` / `build` are green; LLM response extraction smoke is recorded as passed; final real-device smoke for smart draft mode is still required before treating the Builder flow as fully closed. Read `docs/superpowers/plans/2026-04-24-android-profile-builder-handoff.md` before changing Android/Profile Builder behavior; it records the latest Qwen/Tongyi progress, fixes, and blockers.
